...
Operation Operation::fromCSV(const string& line) {
    ...
    if (parts.size() == 4)
        return Operation(parts[0], parts[1], stoi(parts[2]), parts[3]);
    return Operation();
}

string Operation::getClientID() const {
    return clientID;
}
...
string Operation::getDate() const {
    return date;
}
